...
const getSigningKey = async (jwkurl, kid) => {
  ...
  // get JSON Web Key Set from Auth0 url:
  ...
  logger.info('JSON Key Set data ', res);
  ...
  const signingKeys = keys
    .filter(key =>
        ...
        && key.x5c 
        && key.x5c.length // has property 'x5c'
    )
    .map(key => {
  ...
  // find matching key with matching kid:
...
// get the right format of x5c certificate: 
